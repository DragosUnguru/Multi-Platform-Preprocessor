		...
		char **include_paths, size_t no_of_paths)
{
	...
	while (header_ptr == NULL) {
		...
		memcpy(next_header_name, include_paths[path_idx],
			strlen(include_paths[path_idx]) + 1);
	...
	while (fgets(line, MAX_LINE_LEN, header_ptr) != NULL) {
		if (strstr(line, "#include")) {
			...
			ret = resolve_includes(defines, just_name,
						include_paths, no_of_paths);
			DIE(ret != OK, ret);
		} else {
...
int resolve_line(hash_t defines, char *line, FILE *fin)
{
	...
	while (!strncmp(line, "#if ", IF) || !strncmp(line, "#elif ", ELIF)) {
		...
		strncmp(line, "#endif", ENDIF) &&
		strncmp(line, "#elif ", ELIF))
			fgets(line, MAX_LINE_LEN, fin);
	}
...
int process_file(hash_t defines, char **include_paths, int no_paths)
{
	...
	while (fgets(line, MAX_LINE_LEN, stdin) != NULL) {
		if (!strncmp(line, "#include  ", INCLUDE)) {
			...
			ret = resolve_includes(defines, header_name,
...
enum arg_type resolve_input_type(char *arg)
{
	...
	if (arg[0] == '-') {
		...
		result = (input_exists) ? ((input_exists == 1) ?
			OUT_FILE : BAD_PARAM) : IN_FILE;
		++input_exists;
	}
...
int parse_input(int argc, char *argv[], hash_t defines, char ***header_paths,
		size_t *no_header_paths, FILE **inf_ptr, FILE **outf_ptr)
{
	...
	*header_paths = malloc(*no_header_paths * sizeof(**header_paths));
	...
	for (i = 1; i < argc; ++i) {
		...
		if (type == DEFINE || type == STICHED_DEFINE) {
			...
			(*header_paths)[header_idx] =
				malloc(arg_len * sizeof(char));
			DIE((*header_paths)[header_idx] == NULL, FAILURE);

			memcpy((*header_paths)[header_idx],
				arg_value, arg_len - 2);
